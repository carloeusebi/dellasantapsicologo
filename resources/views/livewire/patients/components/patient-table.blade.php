...
<x-custom.table :rows="$patients">
    <x-slot:filters>
        <x-select
            ...
            :options="[
            ['id' => self::ACTIVE_STATE, 'name' => 'Attivi'],
            ['id' => self::ALL_STATE, 'name' => 'Tutti'],
            ['id' => self::ARCHIVED_STATE, 'name' => 'Solo Archiviati'],
        ]"
        />
        @if(auth()->user()->isAdmin())
            <x-select
        ...
        <x-table :rows="$patients" :$headers :row-decoration="$rowDecoration" :$sortBy link="/pazienti/{id}">
            ...
            @unless($patient->isArchived())
                <div class="hidden sm:flex gap-2">
                    <x-button
                        class="btn-xs" icon="o-list-bullet"
                        :link="route('surveys.create', ['patient_id' => $patient->id])"
                    />
                </div>
            @endif
            @endscope
