<div class="grid grid-cols-1 lg:grid-cols-3 items-start gap-8">
    <div class="lg:col-span-2 space-y-8">
        <x-card title="Azioni e dettagli" separator>
          ...
          >Risposte Saltate</span>: {{ $survey->skipped_questions_count }}
                ...
                <div class="flex flex-col order-1 items-end gap-2">
                    <a href="{{ route('download.survey', $survey) }}" target="_blank" class="block w-full xl:btn-wide">
                        <x-button class="w-full xl:btn-wide" icon="o-printer" wire:click="editModal = true">Stampa
                        </x-button>
                    </a>
                ...
                try {
                  ...
                  $wire.dispatch('notify', {type: 'error', title: 'Impossibile copiare il link', description: 'Controlla le autorizzazione del tuo browser'});
                        ...
                        <x-button class="w-full xl:btn-wide" icon="o-envelope" wire:click="emailModal = true">Invia
                            email
                        </x-button>
                    @endif
                    <x-button class="w-full xl:btn-wide" icon="o-trash" wire:click="deleteModal = true">Elimina
                    </x-button>
                </div>
        ...
        <livewire:surveys.components.other-surveys :$survey lazy/>
                ...
                <x-slot:subtitle>
                    <div class="text-xs">
                        ...
                        @elseif ($questionnaireSurvey->has_been_updated)
                            ...
                            <div>
                                {{ $questionnaireSurvey->answers_count }} risposte
                                su {{ $questionnaireSurvey->questions_count }}
                            </div>
                        @else
                            <div>
                                Non Iniziato
    ...
    <div class="full-width-modal">
        <x-modal wire:model="emailModal" title="Invia Email" separator class="backdrop-blur">
            ...
            <x-slot:actions>
                ...
                <x-button wire:click="sendEmail" spinner="sendEmail" icon="o-envelope" class="btn-success">Invia
                </x-button>
            </x-slot:actions>
        </x-modal>
    </div>
